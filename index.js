      ...
      const req = httpModule.request(options, (res) => {
        ...
        res.on('end', () => {
          ...
          if (statusCode >= 300 && statusCode < 400 && res.headers.location) {
            ...
            fetchUrl(redirectUrl, headers, redirectCount + 1)
              .then(resolve)
              .catch(reject);
          } else {
            resolve({
...
async function fetchContent(fetchUrlParam, data, dataType = 'text') {
  try {
    const headers = {};
    ...
    const response = await fetchUrl(fetchUrlParam, headers);
    ...
    return {
      content,
      finalUrl: response.finalUrl || fetchUrlParam,
      ...
      if (line.startsWith('#')) {
        ...
        if (line.includes('URI="')) {
          ...
          if (uriMatch && uriMatch[1]) {
            ...
            const rewrittenUri = `${proxyUrl}/playlist?url=${encodeURIComponent(uri)}&data=${encodeURIComponent(data)}${line.includes('#EXT-X-KEY') ? '&key=true' : ''}`;
        ...
        const fullUrl = `${proxyUrl}/playlist?${urlParam}=${encodeURIComponent(lineUrl)}&data=${encodeURIComponent(data)}${isSegment ? '&type=/index.ts' : '&type=/index.m3u8'}`;
  ...
  try {
    ...
    lines.forEach((line) => {
      if (line.startsWith('#EXTINF')) {
        ...
        if (headerSeparatorIndex !== -1) {
          ...
          const headers = decodedHeadersString
            ? decodedHeadersString.split('&').map(header => {
                ...
                if (cleanKey.toLowerCase() === 'referrer') {
                  cleanKey = 'Referer';
                }
        ...
        if (base64Data) {
          const newUrl = `${baseUrl}/playlist?url=${encodeURIComponent(streamUrl)}&data=${encodeURIComponent(base64Data)}`;
...
// Fetch EPG content with decompression
function fetchEpgContent(epgUrl) {
  return new Promise((resolve, reject) => {
    const client = epgUrl.startsWith('https') ? https : http;

    client.get(epgUrl, (response) => {
...
async function StreamedSUtokenCheck(token) {
  ...
  const urlToCheck = `https://secure.bigcoolersonline.top/check/${token}`;
  ...
  return new Promise((resolve, reject) => {
    const req = https.request(urlToCheck, options, (res) => {
      let data = "";
...
<head>
    ...
    button#add-more, button#fetchPlaylistGroups {
...
<body>
    ...
    <div>
        ...
        <div class="epg_container">
            <input type="checkbox" id="epgMerging" checked><label style="font-size:13px;margin-left:2px;">Merge EPGs</label>
        </div>
...
http://example.com/playlist1.m3u8,http://example.com/playlist2.m3u8,http://example.com/playlist3.m3u8
    ...
    <p>The Select Groups can be used to filter out channels from the playlist based on the group titles. First click on the Choose Groups button to fetch the playlist, the groups will then be listed. Check the box next to each group you want to include in the playlist; any unchecked boxes will be excluded from the playlist.</p>
...
http://example.com/playlist.m3u8
    ...
    <script>
        ...
        document.getElementById('fetchPlaylistGroups').addEventListener('click', async function (event) {
            ...
            async function fetchPlaylist(url) {
                try {
                    ...
                    if (data.includes('<a href="')) {
                        ...
                        if (redirectUrlMatch) {
                            return await fetchPlaylist(redirectUrlMatch[1]);
                        }
                    } 
            ...
            await Promise.all(urls.map(url => fetchPlaylist(url)));

            const groupContainer = document.getElementById('groupCheckboxContainer');
            const labelContainer = document.getElementById('label-checkbox-container');
